<template>
  <div>
    <table class="table-auto select-none w-full shadow-lg" v-if="iData.length > 0">
      <thead>
        <tr>
          ...
          <th class="border px-3 py-2" v-for="item in title" :key="item">{{ item }}</th>
      ...
      <tbody @mouseleave="rowMouse(false)">
        ...
        >
          ...
          <td class="border px-3 py-2" v-for="colItem in title" :key="colItem">{{ item[colItem] }}</td>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const rowMouse = (trigger, event = null, id = -1) => {
      if (event) {
    ...
    const rowClick = (id) => {
      if (selectedRef.value.includes(id)) {
        const modelIndex = selectedRef.value.findIndex((item) => item === id);
        selectedRef.value.splice(modelIndex, 1);
        return;
      }
      selectedRef.value.push(id);
    };
    const rowSelect = (id) => {
      if (!isTrigger.value) {
        return;
    ...
    const funcClick = (type, id) => {
      emit(type, id);
    };

    const multipleClick = (type) => {
      emit(type, selectedRef.value);
    };
    return {
      isTrigger,
      selectedRef,
